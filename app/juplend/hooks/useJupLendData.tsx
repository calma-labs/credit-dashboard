    ...
    if (vaults.length > 0) {
        return vaults
            .filter((v) => {
                ...
                const supplyUsd =
                    (Number(v.totalSupply) /
                        Math.pow(10, v.supplyToken.decimals)) *
                    supplyPrice;
            ...
            .map((v) => {
                ...
                const tvlUsd =
                    ...
                    supplyPrice;
                const borrowUsd =
                    (Number(v.totalBorrow) /
                        Math.pow(10, v.borrowToken.decimals)) *
                    borrowPrice;
                const utilization =
                    tvlUsd > 0
                        ? Number(((borrowUsd / tvlUsd) * 100).toFixed(2))
                        : 0;

                const supplyAPY = Number((v.supplyRate / 100).toFixed(2));
                const borrowAPY = Number((v.borrowRate / 100).toFixed(2));
                const maxLTV = Number(
                    ((v.collateralFactor / 1000) * 100).toFixed(2),
                );
                const lltv = Number(
                    ((v.liquidationThreshold / 1000) * 100).toFixed(2),
                );
