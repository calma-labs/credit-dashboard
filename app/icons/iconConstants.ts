export const STATIC_ICON_MAP: Record<string, string> = {
    ...
    jitosol:
        'https://coin-images.coingecko.com/coins/images/28046/large/JitoSOL_Token_Logo_Green.png',
    ...
    'usd0++':
        'https://coin-images.coingecko.com/coins/images/38663/large/usd0__.png',
...
export const ASSET_ID_OVERRIDES: Record<string, string> = {
    weth: 'weth',
...
export function getCategoryFallbackUrls(key: string): string[] {
    if (key.startsWith('pt-') || key.startsWith('yt-'))
        return [STATIC_ICON_MAP['pendle']];
    if (key.startsWith('mf-') || key === 'marginfi')
        return [STATIC_ICON_MAP['marginfi']];
    if (key === 'jlp' || key === 'juiced' || key.includes('jaaa'))
        return [STATIC_ICON_MAP['jup']];
    if (key.includes('savusd')) return ['https://icons.llama.fi/save.jpg'];
    if (key === 'usp') return [STATIC_ICON_MAP['usdc']];
    if (key === 'stcc' || key === 'cool') return [STATIC_ICON_MAP['sol']];
    if (key === 'ena' || key === 'susde') return [STATIC_ICON_MAP['usde']];
    if (MORPHO_VAULT_PATTERNS.some((p) => key.includes(p)))
        return [STATIC_ICON_MAP['morpho']];
    if (key.includes('usde')) return [STATIC_ICON_MAP['usde']];
    if (key.includes('btc')) return [STATIC_ICON_MAP['btc']];
    if (key.includes('eth')) return [STATIC_ICON_MAP['eth']];
    if (key.includes('sol')) return [STATIC_ICON_MAP['sol']];
    if (key.includes('jup')) return [STATIC_ICON_MAP['jup']];
    if (key.includes('xrp') || key.includes('rlusd'))
        return [STATIC_ICON_MAP['xrp']];
    if (key.includes('comp')) return [STATIC_ICON_MAP['comp']];
    if (key.includes('usds')) return [STATIC_ICON_MAP['usds']];
    if (key.includes('eur')) return [STATIC_ICON_MAP['eurc']];
    if (key.includes('usd') || key.includes('dai'))
        return [STATIC_ICON_MAP['usdc']];
    return [];
}
...
export function isAllowedImageUrl(url: string): boolean {
    try {
        ...
        return (
            hostname.endsWith('.coingecko.com') || hostname === 'icons.llama.fi'
        );
    } catch {
        return false;
    }
...
export function sanitizeImageUrl(
    ...
    if (!url) return null;
    const cleaned = url.replace(/\?[0-9]+$/, '');
    return isAllowedImageUrl(cleaned) ? cleaned : null;
}
