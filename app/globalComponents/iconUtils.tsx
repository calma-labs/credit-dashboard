...
import { STATIC_ICON_MAP, getCategoryFallbackUrls } from "@/app/icons/iconConstants";
...
const assetCache = new Map<string, { promise: Promise<string | null>; expires: number }>();
...
export function ProtocolIcon({
  ...
  if (loading && urls.length === 0) {
    return <span ref={containerRef} className={`inline-block shrink-0 ${className}`} style={{ width: size, height: size }} />;
  }
