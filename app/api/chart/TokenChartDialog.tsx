...
export function TokenDetailView({ symbol, snapshots }: TokenDetailProps) {
  ...
  return (
    <div
      style={{ minHeight: "100vh", background: "#0A0E17", color: "#EEF1F6" }}
    >
      {/* Sticky top bar */}
      <div
        style={{
        ...
        {snapshots.length > 0 && (
          ...
          >
            <StatCard
              ...
              value={
                bestSupplyAPY !== null ? `${bestSupplyAPY.toFixed(2)}%` : "—"
              }
              color="#4FE3C1"
            />
            <StatCard
              ...
              <thead>
                <tr>
                  {[
                    ...
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-[11px] font-semibold text-[#6b7688] font-mono tracking-wider uppercase border-b border-dash-border whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {snapshots.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 align-middle font-mono text-[13.5px] font-semibold text-dash-text">
                      {activeLoans[s.protocol]
                        ? formatTVL(activeLoans[s.protocol])
                        : "—"}
                    </td>
