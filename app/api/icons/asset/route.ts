...
import { STATIC_ICON_MAP, ASSET_ID_OVERRIDES, sanitizeImageUrl } from "@/app/icons/iconConstants";
...
async function fetchByCoinId(coinId: string): Promise<string | null> {
  ...
  return sanitizeImageUrl(data.image?.large) ?? sanitizeImageUrl(data.image?.small) ?? sanitizeImageUrl(data.image?.thumb);
}

async function fetchBySearch(query: string, symbolKey: string): Promise<string | null> {
  const res = await fetch(
...
async function resolveImage(symbol: string): Promise<string | null> {
  ...
  try {
    ...
    cache.set(key, { image: null, expires: Date.now() + NEGATIVE_CACHE_TTL });
    return null;
  }
}
